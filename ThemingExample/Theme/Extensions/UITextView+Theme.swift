...
extension UITextView {
    ...
    private static var __once: () = {
        ...
        let swizzledSelector = #selector(UITextView.ggg_setText(_:));
        
        let originalMethod = class_getInstanceMethod(UITextView.self, originalSelector);
        let swizzledMethod = class_getInstanceMethod(UITextView.self, swizzledSelector);
        
        let didAddMethod = class_addMethod(UITextView.self, originalSelector, method_getImplementation(swizzledMethod!), method_getTypeEncoding(swizzledMethod!));
        
        if didAddMethod {
            class_replaceMethod(UITextView.self, swizzledSelector, method_getImplementation(originalMethod!), method_getTypeEncoding(originalMethod!));
        } else {
